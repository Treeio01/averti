...
const Header = ({ wallet }) => {
    ...
    return (
        <header className="flex w-full justify-between items-center py-3 px-4.5 bg-[#090909] border border-[#1A1A1A] rounded-[16px] flex-wrap gap-3">
                ...
                {NAV_LINKS.map(link => (
                    <li key={link.href}>
                        <Link
                            href={link.href}
                            preserveState
                            className="text-[#EEEEEE]/60 text-[13px] md:text-[15px] hover:text-[#58E8BB] transition-cubic"
                        >
                            {link.label}
                        </Link>
                    </li>
            ...
            <div className="flex items-center gap-[10px] md:gap-[14px]">
                <span className="text-sm text-[#B2B2B4] hidden md:inline">Wallet connected:</span>
