...
const TokenSection = () => (
    <SectionLayout id="token" title="AVRT Token" className="mt-[120px] max-w-[1320px]" isLeft subtitle="The same engine, different economics">
        <div className="flex w-full gap-[20px] flex-wrap justify-center">
            {CARDS.map((card, i) => (
                <div
                    key={card.text}
                    className="flex p-[40px] items-end bg-[#070707] rounded-[25px] border border-[#1A1A1A] min-h-[408px] w-full max-w-[426px] relative overflow-hidden"
                ...
                >
                    ...
                    <span className="text-center text-white font-medium z-50 text-[22px] leading-[140%]">
                        {card.text}
                    </span>
                </div>
            ))}
